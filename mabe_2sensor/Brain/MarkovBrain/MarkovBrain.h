...
public:
  ...
  std::vector<std::vector<double>> STATErecorder;
  void resetSTATErecorder() {
    STATErecorder.clear();
    vector<double> L;
    L.resize(nextNodes.size());
    STATErecorder.push_back(L);
  }
  void recordI() {
    int where=(int)STATErecorder.size()-1;
    for(int i=0;i<nrInputValues;i++){
      STATErecorder[where][i]=inputValues[i];
    }
  }
  void recordO() {
    int where=(int)STATErecorder.size()-1;
    for(int i=0;i<nrOutputValues;i++){
      STATErecorder[where][nrInputValues+i]=nodes[nrInputValues+i];
    }
  }
  void recordH() {
    int where=(int)STATErecorder.size()-1;
    for(int i=0;i<hiddenNodes;i++){
      STATErecorder[where][nrInputValues+nrOutputValues+i]=nodes[nrInputValues+nrOutputValues+i];
    }
  }
  void advanceSTATErecorderTimepoint() {
    vector<double> L;
    L.resize(nodes.size());
    STATErecorder.push_back(L);
  }
  std::vector<std::vector<double>> getRecordedSTATE() {
  ...
  std::pair<std::vector<int>,std::vector<int>> get_T_to_TPO() {
    vector<int> T,TPO;
    bool fbState=DecomposableFeedbackGate::feedbackON;
    int maxReps=5;
    vector<double> recoverState=nodes;
    vector<double> recoverI,recoverO,recoverNext;
    recoverI=inputValues;
    recoverO=outputValues;
    recoverNext=nextNodes;
    DecomposableFeedbackGate::feedbackON=false;
    for(int state=0;state<(1<<nodes.size());state++){
      for(int reps=0;reps<maxReps;reps++){
        for(int i=0;i<nodes.size();i++){
    ...
    //recover current state of everything
    nodes=recoverState;
    nextNodes=recoverNext;
    inputValues=recoverI;
    outputValues=recoverO;
    DecomposableFeedbackGate::feedbackON=fbState;
    return std::pair<std::vector<int>,std::vector<int>>(T,TPO);
  }
  int getBrainStateAsInt() {
    int S=0;
    for(int i=0;i<nodes.size();i++){
      if(nodes[i]>0.0)
        S+=(1<<i);
